...
public class MetamallApplication {
	...
	CommandLineRunner initData(UserRepository userRepository, ProductRepository productRepository, OrderProductRepository orderProductRepository, OrderSheetRepository orderSheetRepository){
		return (args)->{
			...
			User ssar = User.builder().username("ssar").password("1234").email("ssar@nate.com").status(true).role("USER").build();
			User seller = User.builder().username("seller").password("1234").email("seller@nate.com").status(true).role("SELLER").build();
			User admin = User.builder().username("admin").password("1234").email("ssar@nate.com").status(true).role("ADMIN").build();
